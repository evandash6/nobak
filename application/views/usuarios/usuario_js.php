<script>
 var icons = function(cell, formatterParams){
        return "<div class='btn btn-success ver_registro btn-sm' ide='"+cell.getRow().getData().id+"' usuario='"+cell.getRow().getData().usuario+"'fecha_creacion='"+cell.getRow().getData().fecha_creacion+"'id='ver' title='Ver'><i class='fas fa-eye'></i></div> \
                <div class=' editar_registro btn btn-secondary btn-sm' ide='"+cell.getRow().getData().id+"' usuario='"+cell.getRow().getData().usuario+"'fecha_creacion='"+cell.getRow().getData().fecha_creacion+"' id='editar' title='Editar'><i class='fas fa-edit'></i></div> \
    ...
    layout:"fitDataFill",
     ...
     columns:[ //Define Table Columns
		{title:"ID Empleado", field:"id",width:150 },
	 	{title:"Nombre", field:"usuario", width:150},
...
$('body').on('click','.ver_registro',function(){
    ...
    modal_ver(id,usuario,fecha_creacion);
})
...
$('body').on('click','.editar_registro',function(){
    ...
    let usuario = $(this).attr('usuario');
	let fecha_creacion = $(this).attr('fecha_creacion');
    modal_editar(id,usuario,fecha_creacion);
})
...
$('body').on('click','.btx-modificar',function(e){
    ...
    let id = $(this).attr('ide');
    let usuario = $("#usuario_modificado").val();
	let fecha_creacion = $("#fechac_modificada").val();
	console.log(id);
    $.ajax({
        type: "POST",
        url: '<?=base_url()?>inicio/editar_usuario',
        data: {'id':id,'usuario':usuario, 'fecha_creacion':fecha_creacion},
        success: function (data) {
			console.log(id,usuario,fecha_creacion);  
			console.log(data);
